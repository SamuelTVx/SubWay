...
$(document).ready(function() {
	window.addEventListener('message', function(event) {
		if (event.data.action == "open") {
			...
			for (const [key, value] of Object.entries(event.data.station)) {
				myElement = `
					...
					if (value.stationNumber == currentStation) {
						myElement = myElement + `<p class="priceBtn" style="margin-top: 5px;"><i class="fas fa-map-marker-alt"></i> Aktuální Stanice</p>`
					} else {
						myElement = myElement + `Cena: <p class="priceBtn">$${value.price}</p>`
					}
					myElement = myElement + `
								</div>
							</div>
			...
			$("#currentStation").html(`Žádná stance nevybrána`)
			$("#currentPrice").html(`$0`)
			...
			if(intv) { clearInterval(intv);  }
           		intv = setInterval(() => {
           		    curTest = curTest - 1;
           		    $("#timer").html(`${curTest} sekund zbývá...`);
           		}, 1000);
